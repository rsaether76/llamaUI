...
**`qt_app/app/theme.py`**
- Removed custom QSS for `QLineEdit`, `QComboBox`, `QSpinBox`, and `QDoubleSpinBox` so they render with the standard application style.

**`qt_app/app/application.py`**
- Set the application style to `Fusion` (when available) for consistent, standard widget rendering.
